...
class Decomposer(nn.Module):
    ...
    def __init__(self, lights_dim = 4):
        ...
        self.padding = nn.ReplicationPad2d((0,0,6,6))
        ...
        self.lights_fc1 = nn.Linear(16 * 7 * 16, 32) #change 32 * 14 * 32 to 16 * 7 * 16
        # self.lights_fc1 = nn.Linear(32 * 14 * 32, 32)
    ...
    def forward(self, inp):
        ...
        for ind in range(len(self.decoder_lights)):
            ...
            lights = F.leaky_relu(lights, inplace=True)
        lights = lights.view(lights.size(0), -1)
        lights = F.leaky_relu( self.lights_fc1(lights) , inplace=True)
        ...
        return self.remove_padding(reflectance), normed, lights


if __name__ == '__main__':
    inp = Variable(torch.randn(1,3,436,1024))
