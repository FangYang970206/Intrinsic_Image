...
class LastOctaveConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, alpha=0.5, stride=1, padding=1, dilation=1,
        ...
        self.l2h = torch.nn.Conv2d(int(alpha * in_channels), out_channels // 2,
                                   kernel_size, 1, padding, dilation, groups, bias)
        self.h2h = torch.nn.Conv2d(in_channels - int(alpha * in_channels),
                                   out_channels // 2,
                                   kernel_size, 1, padding, dilation, groups, bias)
...
class OctaveCBR(nn.Module):
    def __init__(self,in_channels, out_channels, kernel_size=(3,3),alpha=0.5, stride=1, padding=1, dilation=1,
                 groups=1, bias=False, norm_layer=nn.BatchNorm2d, numGroups=2, activation=nn.LeakyReLU(inplace=True)):
        ...
        if norm_layer == nn.GroupNorm:
            self.bn_h = norm_layer(numGroups, int(out_channels * (1 - alpha)))
            self.bn_l = norm_layer(numGroups, int(out_channels * alpha))
        else:
...
class OctaveCB(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(3,3), alpha=0.5, stride=1, padding=1, dilation=1,
                 groups=1, bias=False, norm_layer=nn.BatchNorm2d, numGroups=2):
...
class LastOCtaveCBR(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(3,3), alpha=0.5, stride=1, padding=1, dilation=1,
        ...
        if norm_layer == nn.GroupNorm:
            self.bn_h = norm_layer(numGroups, out_channels // 2)
        else:
            self.bn_h = norm_layer(out_channels // 2)
...
if __name__ == '__main__':
    ...
    FOCconv = FirstOctaveConv(kernel_size=(3, 3), in_channels=3, out_channels=128, dilation=3).cuda()
    ...
    # test OCB
    ocb = OctaveCB(in_channels=256, out_channels=128, alpha=0.75, norm_layer=nn.GroupNorm).cuda()
    i = high, low
