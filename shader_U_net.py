...
def main():
    ...
    parser.add_argument('--mode',               type=list,  default=['train_sha', 'test_sha'])
    parser.add_argument('--save_path',          type=str,   default='logs\\shader\\',
    ...
    parser.add_argument('--state_dict',         type=str,   default='shader_state.t7')
    ...
    for epoch in range(args.num_epochs):
        ...
        trainer = pipeline.ShaderTrainer(model, train_loader, device, optimizer, writer, step)
        step = trainer.train()
        if args.save_model:
                state = model.state_dict()
                torch.save(state, os.path.join(args.save_path, 'shader_state_{}.t7'.format(epoch)))
        
        test_loss = pipeline.visualize_shader(model, val_loader, device, os.path.join(args.save_path, '{}.png'.format(epoch)))
        writer.add_scalar('test_sha_loss', test_loss, step)
        scheduler.step()
