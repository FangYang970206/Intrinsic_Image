...
class MIT_Trainer:
    def __init__(self, model, loader, lr, device, writer, step, optim_choose='adam'):
        ...
        self.criterion2 = pytorch_ssim.SSIM().to(device)
    ...
    def __epoch(self):
        ...
        for ind, labeled in enumerate(self.loader):
            ...
            lab_inp, lab_refl_targ, lab_shad_targ, lab_mask_targ = labeled
            
            _, lab_refl_pred, lab_shad_pred, shape_pred = self.model.forward(lab_inp)
            print(lab_shad_targ.shape)
            print(lab_shad_pred.shape)
            # lab_recon_pred = lab_recon_pred * lab_mask_targ
            lab_refl_pred = lab_refl_pred * lab_mask_targ
            lab_shad_pred = lab_shad_pred * lab_mask_targ[:, 1, :, :].unsqueeze(1)
            print(lab_shad_pred.shape)
            self.optimizer2.zero_grad()   
            refl_loss_SSIM = (self.criterion2(lab_refl_targ, lab_refl_pred) - 1)**2
            ...
            refl_loss = refl_bf_loss + refl_cos_loss + refl_content_loss*0.1 + refl_loss_SSIM + refl_mse_loss
            ...
            shad_loss_SSIM = (self.criterion2(lab_shad_targ, lab_shad_pred) - 1)**2
            # shad_loss = shad_mse_loss + shape_cos_loss
            shad_loss = shad_bf_loss + shad_content_loss*0.1 + shad_cos_loss + shape_cos_loss + shad_mse_loss + shad_loss_SSIM
            ...
            self.writer.add_scalar('supervision_shape_cos_loss', shape_cos_loss.item(), self.step)
            self.step += 1
            ...
            progress.set_description( '%.5f | %.5f| %.5f| %.5f | %.5f| %.5f' % (refl_cos_loss.item(), shad_cos_loss.item(), refl_bf_loss.item(), shad_bf_loss.item(), refl_content_loss.item(), shad_content_loss.item()) )
