...
class ShapeNetSupervisionTrainer:
    def __init__(self, model, loader, lr, device, writer, step, optim_choose='adam'):
        ...
        self.criterion4 = VGG_Encoder(device)
        self.device = device
        self.writer = writer
        self.step = step
        parameters1 = []
    ...
    def __epoch(self):
        ...
        for ind, labeled in enumerate(self.loader):
            ...
            try:
                lab_inp, lab_refl_targ, lab_shad_targ, _ = labeled
            except ValueError:
            ...
            refl_bf_loss = clc_Loss_albedo(lab_refl_pred, lab_refl_targ, device=self.device)
            ...
            shad_bf_loss = clc_Loss_shading(lab_shad_pred, lab_shad_targ, device=self.device)
    ...
    def update_lr(self, lr):
        for optim in [self.optimizer1, self.optimizer2]:
